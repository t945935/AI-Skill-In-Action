...
走讀範圍：導讀、六個正文單元、五份附錄與六組範例。

## 總結

概念學習路線是連貫的：先盤點，再組合，接著自建、驗證，最後治理與整合案例。真正的卡關不在觀念，而在「範例尚未形成可從乾淨目錄重跑的最小專案」：讀者目前可以讀懂設計，但無法只照書完成一個可驗證的 Skill 發行包。
...
走讀初版是「概念與設計完成、實作教材尚未封口」的版本。本輪已依卡關清單補上可下載、可執行、可看到預期結果的最小案例；後續仍需在 GitHub 乾淨 clone 上做一次獨立回歸走讀。
