...
本專案用於撰寫《AI Skill 實戰：先找、再組、後自建》，教讀者從既有技能盤點開始，逐步完成 Skill 的選型、組合、自建、驗證與發布，並以電子書出版與網頁版教材製作作為完整案例。

## 專案狀態

目前為讀者可用的 `v0.2.0` 候選版。讀者可從 `main` 分支取得範例；正式 tag、授權與商店出版欄位仍在發布閘門中確認。詳細狀態見 [`PROJECT_STATUS.md`](PROJECT_STATUS.md)。
